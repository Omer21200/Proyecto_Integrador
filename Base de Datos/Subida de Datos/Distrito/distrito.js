...
async function buscarTipoLugar(tipo) {
    try {
        ...
        const [rows, fields] = await connection.execute('SELECT * FROM subzona WHERE nombre_subzona = ?', [tipo]);
        ...
        const tipoL = {
            id: rows[0].id_subzona,
            name: rows[0].nombre_subzona
        };
       ...
       //console.log(tipoL);
        ...
        console.error('Error al buscar la subzona: ', error);
        throw error;
    }
}
...
function leerXcel(ruta) {
    ...
    for (const itemfila of dataExcel) {  
        ...
        const pa = {
            ...
            nombre_distrito: itemfila['nombre_distrito'],
            id_subzona: itemfila['id_subzona']
        };
        dat.push(pa); // Convertir el objeto a una cadena JSON
    }
}
leerXcel('Distrito.xlsx');
conectarYInsertarCountryTorneos(dat);
//console.log(dat);
//const tor= buscarTipoLugar("ESMERALDAS");
