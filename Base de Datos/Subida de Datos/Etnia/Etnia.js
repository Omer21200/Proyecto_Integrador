    ...
    try {
        ...
        for (const da of dat) {
            try {
                ...
                const query = `INSERT INTO etnia (nombre_etnia) VALUES ('${da.nombre_etnia}');`;
                await connection.query(query); // Utiliza connection.query en lugar de connection.execute
                console.log(`Detencion "${da.nombre_etnia}" insertada correctamente.`);
                agregarDatosAlArchivo('nombre_etnia.txt', `${query}\n`);
            } catch (error) {
...
function leerXcel(ruta) {
    ...
    for (const itemfila of dataExcel) {
        ...
        */
        ...
        const pa = {
            nombre_etnia: itemfila['nombre_etnia']
        };
        dat.push(pa);
    }
