    ...
    try {
        ...
        for (const da of dat) {
            try {
                ...
                const query = `INSERT INTO zona (nombre_zona) VALUES ('${da.nombre_zona}');`;
                await connection.query(query); // Utiliza connection.query en lugar de connection.execute
                console.log(`zona "${da.nombre_zona}" insertada correctamente.`);
                agregarDatosAlArchivo('zona.txt', `${query}\n`);
            } catch (error) {
...
function leerXcel(ruta) {
    ...
    for (const itemfila of dataExcel) {
        ...
        */
        ...
        const pa = {
            nombre_zona: itemfila['nombre_zona']
        };
        dat.push(pa);
    }
