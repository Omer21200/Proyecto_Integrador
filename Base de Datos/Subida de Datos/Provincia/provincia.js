    ...
    try {
        ...
        for (const da of dat) {
            try {
                ...
                const query = `INSERT INTO provincia (id_provincia,nombre_provincia) VALUES ('${da.id_provincia}','${da.nombre_provincia}');`;
                await connection.query(query); // Utiliza connection.query en lugar de connection.execute
                console.log(`Detencion "${da.nombre_provincia}" insertada correctamente.`);
                agregarDatosAlArchivo('nombre_provincia.txt', `${query}\n`);
            } catch (error) {
    ...
    for (const itemfila of dataExcel) {
        ...
        */
        ...
        const pa = {
            ...
            nombre_provincia: itemfila['nombre_provincia']
        };
        dat.push(pa);
    }
