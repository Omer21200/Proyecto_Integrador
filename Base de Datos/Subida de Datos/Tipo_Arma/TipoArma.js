    ...
    try {
        ...
        for (const da of dat) {
            try {
                ...
                const query = `INSERT INTO tipoarmas (tipo) VALUES ('${da.tipo}');`;
                await connection.query(query); // Utiliza connection.query en lugar de connection.execute
                console.log(`tipo "${da.tipo}" insertada correctamente.`);
                agregarDatosAlArchivo('tipoarmas.txt', `${query}\n`);
            } catch (error) {
