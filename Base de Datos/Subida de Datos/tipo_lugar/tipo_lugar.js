    ...
    try {
        ...
        for (const da of dat) {
            try {
                ...
                const query = `INSERT INTO tipolugar (tipo_lugar) VALUES ('${da.tipo_lugar}');`;
                await connection.query(query); // Utiliza connection.query en lugar de connection.execute
                console.log(`Detencion "${da.tipo_lugar}" insertada correctamente.`);
                agregarDatosAlArchivo('tipo_lugar.txt', `${query}\n`);
            } catch (error) {
...
function leerXcel(ruta) {
    ...
    for (const itemfila of dataExcel) {
        ...
        */
        ...
        const pa = {
            tipo_lugar: itemfila['tipo_lugar']
        };
        dat.push(pa);
    }
