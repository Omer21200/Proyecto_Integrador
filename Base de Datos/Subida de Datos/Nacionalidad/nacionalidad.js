    ...
    try {
        ...
        for (const da of dat) {
            try {
                ...
                const query = `INSERT INTO nacionalidad (tipo_nacionalidad) VALUES ('${da.tipo_nacionalidad}');`;
                await connection.query(query); // Utiliza connection.query en lugar de connection.execute
                console.log(`Detencion "${da.tipo_nacionalidad}" insertada correctamente.`);
                agregarDatosAlArchivo('nacionalidad.txt', `${query}\n`);
            } catch (error) {
...
function leerXcel(ruta) {
    ...
    for (const itemfila of dataExcel) {
        ...
        */
        ...
        const pa = {
            tipo_nacionalidad: itemfila['tipo_nacionalidad']
        };
        dat.push(pa);
    }
